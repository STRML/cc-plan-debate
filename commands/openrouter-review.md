...
allowed-tools: Bash(bash ~/.claude/debate-scripts/debate-setup.sh:*), Bash(bash ~/.claude/debate-scripts/invoke-openai-compat.sh:*), Bash(rm -rf /private/tmp/claude/ai-review-:*), Write(/private/tmp/claude/ai-review-*), TeamCreate, TeamDelete, SendMessage, Agent
---

# AI Multi-Model Plan Review (OpenRouter)

Run all configured AI reviewers in parallel via OpenRouter, synthesize their feedback, debate contradictions, and produce a final consensus verdict. Max 3 total revision rounds.
...
Read `~/.claude/debate-openrouter.json`. If missing, stop:
```text
Config not found: ~/.claude/debate-openrouter.json
Run /debate:openrouter-setup to create it.
```
...
```
TeamCreate: name="openrouter-<REVIEW_ID>", description="Parallel OpenRouter plan review"
```
...
For each reviewer in the config, use the Agent tool with `team_name: "openrouter-<REVIEW_ID>"`. Spawn all in parallel.
...
"Run `bash <SCRIPT_DIR>/invoke-openai-compat.sh "~/.claude/debate-openrouter.json" "<WORK_DIR>" "<name>"` (use timeout: 360000). Read `<WORK_DIR>/<name>-exit.txt` for the exit code. After completion, delete `<WORK_DIR>/<name>-prompt.txt`."
...
```text
## OpenRouter Review — UNDECIDED

All reviewers failed or timed out. No synthesis is possible.

Options:
- Check API key and model availability with /debate:openrouter-setup
- Re-run /debate:openrouter-review
```
...
**Team mode:** SendMessage the teammate with instructions to re-run the invoke script:
```
SendMessage:
  Recipient: "<name>-reviewer"
  Content:
    "A debate prompt has been written to <WORK_DIR>/<name>-prompt.txt.
     Re-run: bash <SCRIPT_DIR>/invoke-openai-compat.sh "~/.claude/debate-openrouter.json" "<WORK_DIR>" "<name>"
     Read <WORK_DIR>/<name>-exit.txt and report back.
     After completion, delete <WORK_DIR>/<name>-prompt.txt."
```

**Agent mode:** Spawn a fresh agent with `run_in_background: true` that calls the invoke script, reads the exit file, and reports.
...
```text
---
## OpenRouter Review — Final Report (Round N of 3)
...
```text
## OpenRouter Review — Max Rounds Reached
...
- **No provider CLIs needed.** Everything goes through OpenRouter's OpenAI-compatible API via curl.
- **No session resume.** Each round is stateless — full context is injected via prompt files.
- **Config is king.** Adding a reviewer = adding an entry to `~/.claude/debate-openrouter.json`.
