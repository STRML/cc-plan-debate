...
allowed-tools: Bash(bash ~/.claude/debate-scripts/debate-setup.sh:*), Bash(bash ~/.claude/debate-scripts/invoke-gemini.sh:*), Bash(rm -rf .claude/tmp/ai-review-:*), Bash(which gemini:*), Bash(gemini -s:*)
---
...
1. Read `.claude/tmp/ai-review-${REVIEW_ID}/gemini-output.md`
...
1. **Revise the plan** — address each issue Gemini raised. Update the plan content in the conversation context and rewrite `.claude/tmp/ai-review-${REVIEW_ID}/plan.md` with the revised version.
...
} > .claude/tmp/ai-review-${REVIEW_ID}/gemini-prompt.txt
